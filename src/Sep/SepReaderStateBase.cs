    ...
    where TCharInfo : struct, ISepCharInfo<TChar>
{
    ...
    internal System.Globalization.CultureInfo? _cultureInfo;

    internal TChar[] _chars = Array.Empty<TChar>();
...
#pragma warning restore CA2213 // Disposable fields should be disposed
    ...
    internal SepReaderStateBase(bool colUnquoteUnescape = false, SepTrim trim = SepTrim.None)
    {
    ...
    // Constructor for copy/clone scenarios where flags are copied directly
    internal SepReaderStateBase(uint colSpanFlags)
    {
        _colSpanFlags = colSpanFlags;
    }

    internal void SwapParsedRowsBuffersTo(SepReaderStateBase<TChar, TCharInfo> other)
    {
    ...
    internal static void EnsureArrayFromPoolHasMinimumLength<T>(ref T[] array, int minimumLength)
    {
    ...
    void ThrowInvalidDataExceptionColCountMismatch(int colCountExpected)
    {
        ...
        var row = CreateRowString(rowStart, rowEnd);
    ...
    internal virtual string CreateRowString(int rowStart, int rowEnd)
    {
        // Default implementation for char; overridden for byte (UTF-8 decode)
        if (typeof(TChar) == typeof(char))
        {
            var chars = Unsafe.As<TChar[], char[]>(ref _chars);
            return new string(chars, rowStart, rowEnd - rowStart);
        }
        else if (typeof(TChar) == typeof(byte))
        {
            var bytes = Unsafe.As<TChar[], byte[]>(ref _chars);
            return System.Text.Encoding.UTF8.GetString(bytes, rowStart, rowEnd - rowStart);
        }
        return string.Empty;
    }
    ...
    internal ReadOnlySpan<TChar> GetColSpan(int index)
    {
        ...
        else if (_colSpanFlags == UnescapeFlag) // Unquote/Unescape
        {
            ...
            if (quoteCountOrNegativeUnescapedLength == 0 ||
                (quoteCountOrNegativeUnescapedLength > 0 && !colRef.Equals(TCharInfo.Quote)))
            {
            ...
            // already unescaped. Optimize for common case of outermost quotes.
                     ...
                     Unsafe.Add(ref colRef, colLength - 1).Equals(TCharInfo.Quote))
            {
            ...
            else
            {
                ...
                var unescapedLength = SepUnescape.UnescapeInPlace<TChar, TCharInfo>(ref colRef, colLength);
    ...
    [MethodImpl(MethodImplOptions.NoInlining)]
    ReadOnlySpan<TChar> GetColSpanTrimmed(int index)
    {
        ...
        else
        {
            ...
            if (quoteCountOrNegativeUnescapedLength == 2 &&
                colRef.Equals(TCharInfo.Quote) && Unsafe.Add(ref colRef, colLength - 1).Equals(TCharInfo.Quote))
            {
                colRef = ref Unsafe.Add(ref colRef, 1);
                colLength -= 2;
            }
            else if (colLength > 0 && colRef.Equals(TCharInfo.Quote))
            {
                colLength = SepUnescape.TrimUnescapeInPlace<TChar, TCharInfo>(ref colRef, colLength);
    ...
    SepRange GetColSpanTrimmedRange(int index)
    {
        ...
        var colStart = (int)(byteOffset / Unsafe.SizeOf<TChar>());
    ...
    {
        var start = 0;
        while (start < length && Unsafe.Add(ref col, start).Equals(TCharInfo.Space))
        { start++; }

        var end = length - 1;
        while (end >= start && Unsafe.Add(ref col, end).Equals(TCharInfo.Space))
        { end--; }
    ...
    {
        ArrayPool<TChar>.Shared.Return(_chars);
