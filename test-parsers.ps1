...
try {
    ...
    foreach ($parser in $parsers) {
        ...
        dotnet test --project .\src\Sep.Test\Sep.Test.csproj --nologo -c Debug --no-build --no-restore
        if ($LASTEXITCODE -ne 0) {
            throw "Debug parser test failed for $parser with exit code $LASTEXITCODE."
        }
        Write-Output "Testing $parser Release"
        dotnet test --project .\src\Sep.Test\Sep.Test.csproj --nologo -c Release --no-build --no-restore
        if ($LASTEXITCODE -ne 0) {
