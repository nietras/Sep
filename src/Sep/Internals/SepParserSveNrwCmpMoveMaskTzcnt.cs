...
using VecUI16 = System.Numerics.Vector<ushort>;
using VecUI8 = System.Numerics.Vector<byte>;
...
sealed class SepParserSveNrwCmpMoveMaskTzcnt : ISepParser
{
    ...
    {
        if (VecUI8.Count == Vector64<byte>.Count)
        {
            return Vector64.ExtractMostSignificantBits(As<VecUI8, Vector64<byte>>(ref v));
        }
        if (VecUI8.Count == Vector128<byte>.Count)
        {
            return Vector128.ExtractMostSignificantBits(As<VecUI8, Vector128<byte>>(ref v));
        }
        if (VecUI8.Count == Vector256<byte>.Count)
        {
            return Vector256.ExtractMostSignificantBits(As<VecUI8, Vector256<byte>>(ref v));
        }
        if (VecUI8.Count == Vector512<byte>.Count)
        {
