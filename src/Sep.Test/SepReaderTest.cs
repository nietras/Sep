...
public partial class SepReaderTest
{
    ...
    [DataRow(false)]
    [TestMethod]
    public async ValueTask SepReaderTest_Extensions_FromStream_LeaveOpen(bool leaveOpen)
    {
        {
            var stream = new MemoryStream(Encoding.UTF8.GetBytes("A\n1\n"));
            using (var reader = Sep.Reader().From(stream, leaveOpen))
            {
                Assert.IsTrue(reader.MoveNext());
        ...
        {
            var stream = new MemoryStream(Encoding.UTF8.GetBytes("A\n1\n"));
            using (var reader = await Sep.Reader().FromAsync(stream, leaveOpen))
            {
