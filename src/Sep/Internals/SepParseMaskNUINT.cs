...
static partial class SepParseMaskNUINT
{
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal static ref int ParseSeparatorsMask(nuint mask, nuint charsIndex, ref int colEndsRef)
    ...
    internal static ref int ParseAnyCharsMask(nuint mask, char separator,
        scoped ref char charsRef, nuint charsIndex,
        scoped ref nuint rowLineEndingOffset, scoped ref nuint quoteCount,
        ref int colEndsRef, scoped ref nuint lineNumber)
    ...
    internal static ref int ParseSeparatorsLineEndingsMasks(nuint separatorsMask, nuint separatorsLineEndingsMask,
        scoped ref char charsRef, scoped ref nuint charsIndex, char separator,
        ref int colEndsRefCurrent, scoped ref nuint rowLineEndingOffset, scoped ref nuint lineNumber)
    ...
    internal static ref TColInfo ParseSeparatorsMask<TColInfo, TColInfoMethods>(
        nuint mask, nuint charsIndex, ref TColInfo colInfosRef)
        where TColInfoMethods : ISepColInfoMethods<TColInfo>
    {
        do
        {
            ...
            colInfosRef = TColInfoMethods.Create((int)charsIndex + relativeIndex, 0);
        }
        while (mask != 0);
    ...
    internal static ref TColInfo ParseAnyCharsMask<TColInfo, TColInfoMethods>(
        ...
        scoped ref char charsRef, nuint charsIndex,
        scoped ref nuint rowLineEndingOffset, scoped ref nuint quoteCount,
        ref TColInfo colInfosRef, scoped ref nuint lineNumber)
        where TColInfoMethods : ISepColInfoMethods<TColInfo>
    {
        do
        {
    ...
    internal static ref TColInfo ParseAnyChar<TColInfo, TColInfoMethods>(
        scoped ref char charsRef, nuint charsIndex, int relativeIndex, char separator,
        scoped ref nuint rowLineEndingOffset, scoped ref nuint quoteCount,
        ref TColInfo colInfosRef, scoped ref nuint lineNumber)
        where TColInfoMethods : ISepColInfoMethods<TColInfo>
    {
    ...
    ADDCOLEND:
        // Pre-increment colInfosRef since [0] reserved for row start
        colInfosRef = ref Add(ref colInfosRef, 1);
        colInfosRef = TColInfoMethods.Create((int)charsIndex + relativeIndex, (int)quoteCount);
        quoteCount = 0;
    RETURN:
        return ref colInfosRef;
    }
    ...
    internal static ref TColInfo ParseSeparatorsLineEndingsMasks<TColInfo, TColInfoMethods>(
        ...
        scoped ref char charsRef, scoped ref nuint charsIndex, char separator,
        ref TColInfo colInfosRefCurrent, scoped ref nuint rowLineEndingOffset, scoped ref nuint lineNumber)
        where TColInfoMethods : ISepColInfoMethods<TColInfo>
    {
        if (separatorsMask == 0)
        {
        ...
        else
        {
            ...
            if (((SizeOf<nuint>() * 8 - 1) - BitOperations.LeadingZeroCount(separatorsMask)) < lineEndingIndex)
            {
                ...
                charsIndex += (uint)lineEndingIndex;
                colInfosRefCurrent = TColInfoMethods.Create((int)charsIndex, 0);
                if (c == CarriageReturn)
                {
                    // If \r=CR, we should always be able to look 1 ahead, and if char not valid should not be \n=LF
            ...
            else
            {
                ...
                charsIndex = (uint)TColInfoMethods.GetColEnd(colInfosRefCurrent) + rowLineEndingOffset;
            }
        }
    ...
    internal static ref TColInfo ParseLineEndingMask<TColInfo, TColInfoMethods>(
        ...
        scoped ref char charsRef, scoped ref nuint charsIndex,
        ref TColInfo colInfosRefCurrent, scoped ref nuint rowLineEndingOffset, scoped ref nuint lineNumber)
        where TColInfoMethods : ISepColInfoMethods<TColInfo>
    {
    ...
    static ref TColInfo ParseSeparatorsLineEndingsMask<TColInfo, TColInfoMethods>(
        nuint mask, char separator,
        scoped ref char charsRef, nuint charsIndex,
        scoped ref nuint rowLineEndingOffset,
        ref TColInfo colInfosRef, scoped ref nuint lineNumber)
        where TColInfoMethods : ISepColInfoMethods<TColInfo>
    {
        do
